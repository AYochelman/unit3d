...
**חריג אחד, מבוקש (18.9):** מודל שהבעלים שמר לאוסף **משלו** מאושר לבד בסנכרון
היומי — השמירה היא ה"כן". `sync:collections -- --publish` כותב לו "approved"
לתוך `public/model-decisions.json`, אותו קובץ ש-/admin כותב, ו-`apply:approvals`
בונה ממנו את שורת המדף. מסלול אחד למדף, לא משנה מי אמר כן.

שני דברים נשארים מאחורי השער גם במצב הזה:
- **לייק הוא לא החלטה.** הוא ממשיך לתור. ככה זוכרים משהו, לא ככה בוחרים למכור.
- **כל אזהרה של `holdsFor`** — רישיון NC, נשק, מותג — ממתינה לו, בכל אוסף.
  אלה בדיוק המקרים שבהם אישור מתוך הרגל הוא איך שחנות מוכרת משהו שאסור לה.
...
## Reference Studio — ספריית הרפרנסים (`tools/reference-studio`)

כלי מקומי לחד-משתמש: אוספים תמונות רפרנס וקישורי אתרים, מסמנים מה אוהבים בכל אחד,
והכלי מייצר מהם בריף עיצוב, פרומפט לתמונה, טוקנים, ובריף פרויקט שלם עם פרומפט
מוכן ל-Claude Code.

```bash
npm run studio        # http://localhost:3100
npm run studio:check  # בדיקה עצמית (‎-- --live גם מול שרת שרץ)
```

**למה זו אפליקציית Next נפרדת ולא מסלול באתר:** האתר נבנה כ-static export ל-GitHub
...
**קישור לאתר** נפתח בדפדפן אמיתי ומחזיר צילומי דסקטופ ומובייל, וגם **מדידות מהדף
החי** — פונטים שבאמת הוחלו, צבעים לפי שטח, סולם הכותרות, רדיוסים, ספירת מעברים
ואנימציות, ו-breakpoints. זה מה שמאפשר לומר על אתר שנתפס "פונט X — נמדד", בעוד
שמצילום מסך לבדו אף פעם אי אפשר.

**ניתוח** רץ בשני מצבים בלבד, והממשק אומר איזה: עם מפתח API — בקשת vision אמיתית;
בלי מפתח — ייצוא חבילה, הרצה ב-Claude Code, וייבוא ה-JSON. אין מצב שלישי שממציא
תיאור, ואין כפתור שלא עושה כלום. ולידטור אחד משרת את שני המסלולים ואוכף: על תמונה
סטטית שום דבר לא יכול להיות "נמדד" — פונט, אנימציה וצבע יורדים ל"הערכה" עם הסבר.

`tools/reference-studio/README.md` — המדריך המלא.
