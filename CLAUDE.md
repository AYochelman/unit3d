...
**בקשה אחת עוברת, וכל השאר 403 — זה עוגייה, לא חסימה.** Cloudflare מחזיר
`set-cookie: __cf_bm` בפנייה הראשונה ומצפה לקבל אותה חזרה. ל-`fetch` של node
אין מגירת עוגיות, ולכן כל בקשה הגיעה בלעדיה והוחזרה 403. `getJson` ב-
`scripts/lib/makerworld.mjs` שומר את העוגיות למשך התהליך ומחזיר אותן — זה
המקום היחיד שצריך לתקן, כי כל הסקריפטים עוברים דרכו. אם זה חוזר: לבדוק מול
דפדפן אמיתי על אותם ids לפני שמאשימים את הכתובת או הגבלת קצב.
