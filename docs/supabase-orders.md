...
**כדי שהמיילים לא ינחתו בספאם** — לשלוח מכתובת בדומיין unit-3d.com דרך Brevo
ולא מ-Gmail. ההתקנה, רשומות ה-DNS והחיבור ל-EmailJS: `docs/email-deliverability.md`.
בתבנית ב-EmailJS, השדה **Reply To** צריך להיות `{{reply_to}}` — הקוד שולח בו
את כתובת המייל של החנות, כך שתשובה של לקוח מגיעה אליך ולא לשירות השליחה.
...
# מיילים ללקוח לאורך הדרך (עלתה למדפסת · מוכנה)

הלקוח מקבל שלושה מיילים: אישור כשהוא שולח, **"אושרה ועולה למדפסת"** עם קישור
לשידור החי ברגע האישור באדמין, ו"מוכנה" כשהפריט האחרון מסומן. שני האחרונים
יוצאים מהדפדפן של אריאל, פעם אחת בלבד לכל הזמנה — החותמת נשמרת בשורה:

```sql
alter table public.orders
  add column if not exists live_email_at  timestamptz,
  add column if not exists ready_email_at timestamptz;
```

בלי העמודות ההחלטה והסימון עדיין נשמרים; רק החותמת נופלת, והאדמין יציע
לשלוח שוב. הערה שנכתבת עם האישור נכנסת לגוף המייל.

---

# מספר מעקב וחברת שליחויות

כשהחבילה יוצאת, האדמין בוחר חברה ומזין מספר מעקב. זה נשמר בשורת ההזמנה,
והמייל "מוכנה" נושא את המספר — ואם החברה מוכרת לנו, גם קישור שהלקוח לוחץ.
...
המבנה: `{"courier":"israel-post","code":"RR123456789IL","url":null,"at":"..."}`.

**למה לא לכל חברה יש קישור.** `lib/couriers.ts` בונה קישור רק לחברות שכתובת
דף המעקב שלהן ידועה — דואר ישראל, UPS, FedEx, DHL. לצ'יטה, HFD ובלדר יש דף
מעקב שכתובתו אינה משהו לנחש: קישור שנבנה מתבנית שגויה נראה בדיוק כמו קישור
תקין עד שהלקוח לוחץ ונוחת בשגיאה. לחברות האלה נפתח שדה שבו מדביקים את
הקישור האמיתי פעם אחת, ואם אין — המייל מציג את המספר בלבד.

בלי העמודה שאר ההזמנה עובדת כרגיל; רק השמירה הזו תיכשל, והאדמין יגיד את זה
במפורש.
...
## מתי הלקוח קיבל הודעה שההזמנה מוכנה

ברגע שמסמנים את הפריט האחרון בהזמנה כמוכן, האדמין שולח ללקוח מייל ("מוכנה
לאיסוף" או "יוצאת למשלוח", לפי אופן המסירה). כדי שזה יקרה **פעם אחת בלבד** —
ולא שוב בכל פעם שמסמנים ומבטלים פריט — נשמר חותם זמן על ההזמנה:
...
בלי העמודה הזאת המייל עדיין נשלח והכל עובד; מה שלא עובד הוא הזיכרון — האתר לא
ידע שכבר הודיע, ובכל סימון מחדש הלקוח יקבל מייל נוסף. הכתיבה של החותם היא
בקשה נפרדת משלה בכוונה, כך שעמודה חסרה מפילה את הזיכרון ולא את סימון הפריט
עצמו.
